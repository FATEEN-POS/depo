...
function buildGamificationHtml() {
  ...
  if (topCommission && topCommission.commission > 0) badges.push({icon:'🏆', label:'أعلى تحصيل عمولة', name:topCommission.name, value:`${fmt(topCommission.commission)} ج.م`});
  const topClients = [...team].sort((a,b)=>b.clients-a.clients)[0];
  if (topClients && topClients.clients > 0) badges.push({icon:'👥', label:'أكتر عملاء', name:topClients.name, value:`${topClients.clients} عميل`});
  ...
  if (bestCloseRate) badges.push({icon:'🎯', label:'أعلى نسبة إغلاق', name:bestCloseRate.name, value:`${Math.round(bestCloseRate.rate*100)}%`});
  const lowestCancel = [...withRate].sort((a,b)=>(a.cancelled/a.clients)-(b.cancelled/b.clients))[0];
  if (lowestCancel) badges.push({icon:'🛡️', label:'أقل نسبة إلغاء', name:lowestCancel.name, value:`${Math.round((lowestCancel.cancelled/lowestCancel.clients)*100)}%`});
  ...
  if (me) {
    ...
    <div style="margin-top:16px;padding-top:14px;border-top:1px dashed var(--border2)">
      <div style="font-weight:800;font-size:13px;margin-bottom:10px">🎖️ شاراتي أنا</div>
      ...
      ${myBadges.length ? `<div class="gamif-badges-wrap">${myBadges.map(b => `
        <div class="gamif-badge">
          <div class="gamif-badge-icon">${b.icon}</div>
          <div class="gamif-badge-body"><div class="gamif-badge-label">${b.label}</div><div class="gamif-badge-name" style="color:var(--green)">مبروك، إنت الأول! 🎉</div></div>
          <div class="gamif-badge-value">${b.value}</div>
          ...
          <div style="text-align:center">
            ...
            ${autoCommThisMonth > 0 ? (
              (m.commissionAddedMonths || []).includes(todayISO().slice(0,7))
                ...
                : `<button class="btn btn-outline btn-sm" style="margin-top:4px;padding:2px 8px;font-size:10.5px" onclick="addAutoCommissionToBalance(${JSON.stringify(m.id)})">➕ ضيفها للرصيد</button>`
            ) : ''}
          </div>
        </div>
        <div class="actions">
...
function renderWhatsappReminders() {
  ...
  wrap.innerHTML = `
    <div class="sec-header"><div class="sec-title">💬 تذكيرات واتساب اليوم — مبالغ متبقية</div></div>
...
async function scanBusinessCard(e) {
  ...
  statusEl.textContent = 'جاري قراءة الكارت... (ممكن تاخد شوية ثواني)';
  try {
    if (typeof Tesseract === 'undefined') {
      statusEl.textContent = 'تعذّر تحميل أداة القراءة — تأكد من اتصال الإنترنت';
      return;
    }
    ...
    statusEl.textContent = (phoneMatch || nameLine) ? '✅ تم استخراج البيانات — راجعها قبل الحفظ' : '⚠️ مقدرش يستخرج بيانات واضحة، اكتبها يدويًا';
  } catch (err) {
    statusEl.textContent = '⚠️ حصل خطأ أثناء قراءة الكارت';
  } finally {
    e.target.value = '';
  }
}
...
function saveFeedback() {
  ...
  if (currentFbRating === 1) {
    ...
    toast(`⚠️ تقييم نجمة واحدة من ${client ? client.name : 'عميل'} — المسؤول: ${agentName}`, 'error');
  }
  renderFeedbackPage();
}
...
function checkPhoneDuplicate() {
  ...
  if (dup) {
    warnEl.textContent = `⚠️ الرقم ده مسجل بالفعل عند "${dup.name}"`;
...
// ── إرسال رابط تقييم ذاتي للعميل عبر واتساب (بدل ما الموظف يسجل التقييم بنفسه) ──
// الفكرة: التطبيق ده من غير سيرفر خلفي، فمينفعش رابط "يحفظ التقييم تلقائي" في قاعدة بياناتنا.
// الحل: صفحة rate.html مستقلة (اتبعتلك كمان) بتاخد تقييم العميل، وبعدين بتفتحله واتساب
// برسالة جاهزة يبعتها هو بنفسه لرقم شركتك — فبدل ما الموظف يدخل التقييم، العميل يبعته مباشرة.
function sendFeedbackLinkToClient() {
  const clientId = parseInt(document.getElementById('fb-client').value);
  const c = DB.clients.find(x => x.id === clientId);
  if (!c) { toast('اختار العميل الأول', 'error'); return; }
  if (!c.phone) { toast('العميل ده مفيهوش رقم تليفون مسجّل', 'error'); return; }
  const s = DB.settings || {};
  if (!s.feedbackPageUrl) {
    toast('لازم تحط رابط صفحة التقييم في الإعدادات الأول (بعد ما ترفع rate.html لأي استضافة)', 'error');
    return;
  }
  if (!s.phone) {
    toast('لازم تحط رقم تليفون شركتك في الإعدادات الأول عشان تقييم العميل يوصلك واتساب', 'error');
    return;
  }
  const params = new URLSearchParams({
    company: s.brandName || '',
    phone: cleanPhoneForWa(s.phone),
    client: c.name,
    service: c.service || ''
  });
  const link = `${s.feedbackPageUrl}?${params.toString()}`;
  const text = `مرحباً "${c.name}" 🌹\nياريت تقيّم خدمة "${c.service || ''}" اللي استلمتها من ${s.brandName || ''} من خلال الرابط ده:\n${link}`;
  const url = `https://wa.me/${cleanPhoneForWa(c.phone)}?text=${encodeURIComponent(text)}`;
  window.open(url, '_blank');
  logActivity('إرسال رابط تقييم واتساب', c.name);
  toast('اتفتح واتساب — دوس إرسال عشان الرابط يوصل للعميل');
}
...
function renderSmartFollowupSuggestions() {
  ...
  if (!suggestions.length) {
    box.innerHTML = `<div class="empty" style="padding:24px"><div style="font-size:13px">مفيش عملاء معدّيين ${threshold} يوم من غير تفاعل — كله تمام 👌</div></div>`;
    return;
  }
...
function renderSettings() {
  ...
  document.getElementById('sett-feedback-url').value = s.feedbackPageUrl || '';
...
function saveBrandInfo() {
  ...
  DB.settings.feedbackPageUrl = document.getElementById('sett-feedback-url').value.trim();
